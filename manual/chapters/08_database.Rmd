...
knitr::include_graphics("diagrams/schema.png")
```
...
![pgAdmin showing the dialog to add a new server, note that here we are adding the localhost](figures/pg_admin_add_server.png)
...
![pgAdmin showing the dialog to create a new database, note that here we are creating the `template_masim` template database](figures/pg_admin_create.png)

![pgAdmin showing the results of the `database.sql` script being run. Note that tables have been created under the `sim` schema.](figures/pg_admin_script.png)
