...
In the event that a database backup is required then it is recommended to use the pgAdmin control panel to start the process via `Tools > Backup` when a database is selected. A dialog will appear that will allow a file name can then be supplied (ex., `backup.bak`) and by default the file will be written to `/var/lib/pgadmin/storage/USERNAME/backup.bak` where `USERNAME` is the pgAdmin login of the user that started the process. Typically this is a protected directory so it is recommended to login to the server and move the backup to a different directory. Once moved, `rsync` can be used to move the file:
