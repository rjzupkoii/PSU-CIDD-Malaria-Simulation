...
stream = open('input_FLAL_OZ439_no_importation.yml', 'r');
...
for mda_round in number_MDA_round:
    for beta in betas:
        for _,itc in improved_tc.items():                
            ...
            output_filename = 'FLAL_OZ439_no_importation/%s/ONELOC_%s_%dRMDA_%s_OPPUNIFORM_FLAL%s.yml'%(kFormatter(popsize), kFormatter(popsize),mda_round,pfpr_str,itc);
