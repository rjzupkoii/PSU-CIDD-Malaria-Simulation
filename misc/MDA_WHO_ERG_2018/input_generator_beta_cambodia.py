...
for i in range(number_of_beta_sets):
    betas.append(loguniform(log(0.05),log(0.5),number_of_locations))


#%%
for index,beta in enumerate(betas):
    data['location_db']['beta_by_location'] = np.full(number_of_locations, beta).tolist()
    output_filename = 'Cambodia/beta/input_beta_%d.yml'%index;
...
numpy.savetxt("Cambodia/beta.csv", a, delimiter=",")
#
#
