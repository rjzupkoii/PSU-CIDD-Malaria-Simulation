...
for fl, input_fn in first_line.items():            
    ...
    for infection_duration_factor_str, infection_duration_factor in infection_duration_factors.items():
            ...
            for comp, comp_str in compliance.items():        
                for tc, tc_map in treatment_coverages.items():            
                    for beta, pfpr in tc_map['beta_pfpr'].items():
                        ...
                        new_data['immune_system_information']['duration_for_naive'] = new_data['immune_system_information']['duration_for_naive'] * infection_duration_factor
                        new_data['immune_system_information']['duration_for_fully_immune'] = new_data['immune_system_information']['duration_for_fully_immune'] * infection_duration_factor
                        ...
                        ## save to file
                        output_filename = 'SA3/%s_infection_duration_factor_%s_mu_0p001983_introduce_mutant_%s_comp_%s_tc_%s_pfpr_%s.yml'%( fl, infection_duration_factor_str ,mu_str, comp_str, tc_map['f'], pfpr);
