...
//      DoubleVector2(number_of_location, DoubleVector(number_of_parasite_type, 0));
    ...
    current_force_of_infection_by_location_parasite_type_v2_ =
      DoubleVector2(number_of_location, DoubleVector(number_of_parasite_type, 0));
      ...
      for (auto loc = 0; loc < Model::CONFIG->number_of_locations(); loc++) {
        for (auto genotype = 0; genotype < Model::CONFIG->number_of_parasite_types(); genotype++) {
          force_of_infection_for7days_by_location_parasite_type_[d][loc][genotype] =
            current_force_of_infection_by_location_parasite_type_v2_[loc][genotype];
        }
      }
    }
  }
...
void Population::update_force_of_infection(const int &current_time) {
  ...
  for (auto loc = 0; loc < Model::CONFIG->number_of_locations(); loc++) {
    for (auto p_type = 0; p_type < Model::CONFIG->number_of_parasite_types(); p_type++) {
      ...
      //reset for next day
      current_force_of_infection_by_location_parasite_type_v2_[loc][p_type] = 0;
    }
  }
}
  ...
  for (auto loc = 0; loc < Model::CONFIG->number_of_locations(); loc++) {
    for (auto parasite_type_id = 0;
      ...
      interupted_feeding_force_of_infection_by_location_parasite_type_[loc][parasite_type_id] =
        current_force_of_infection_by_location_parasite_type_v2_[loc][parasite_type_id];
...
#pragma omp parallel for
          for (auto p_id = 0; p_id < Model::CONFIG->number_of_parasite_types(); p_id++) {
            current_force_of_infection_by_location_parasite_type_v2_[loc][p_id] +=
              ...
              p->relative_infectivity(p->all_clonal_parasite_populations()->log10_total_relative_density()) *
              p->all_clonal_parasite_populations()->relative_effective_parasite_density()->at(p_id);
          }
