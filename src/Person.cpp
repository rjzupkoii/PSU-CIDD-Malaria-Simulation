...
void Person::update_bitting_level() {
  if (Model::CONFIG->using_age_dependent_bitting_level()) {
    ...
    double new_bitting_level_value = base_bitting_level_value_ * get_age_dependent_biting_factor();
...
double Person::get_age_dependent_biting_factor() {
  //
  //0.00 - 0.25  -  6.5
  //0.25 - 0.50  -  8.0
  //0.50 - 0.75  -  9.0
...
// TODO: calculate age by actual calendar
  ...
  if (age_ < 1) {
    double age = ((Model::SCHEDULER->current_time() - birthday_) % Constants::DAYS_IN_YEAR()) /
