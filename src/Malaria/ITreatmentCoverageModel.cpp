...
ITreatmentCoverageModel* ITreatmentCoverageModel::build_steady_tcm(const YAML::Node& node, Config* config) {
  ...
  read_p_treatment(node["p_treatment_for_less_than_5_by_location"], result->p_treatment_less_than_5, config->number_of_locations());
  read_p_treatment(node["p_treatment_for_more_than_5_by_location"], result->p_treatment_more_than_5, config->number_of_locations());


  return result;
}

void ITreatmentCoverageModel::read_p_treatment(const YAML::Node& node, std::vector<double>& p_treatments, const int number_of_locations) {
  if (node) {
...
ITreatmentCoverageModel* ITreatmentCoverageModel::build_linear_tcm(const YAML::Node& node, Config* config) {
  ...
  read_p_treatment(node["p_treatment_for_less_than_5_by_location_from"], result->p_treatment_less_than_5, config->number_of_locations());
  read_p_treatment(node["p_treatment_for_more_than_5_by_location_from"], result->p_treatment_more_than_5, config->number_of_locations());

  read_p_treatment(node["p_treatment_for_less_than_5_by_location_to"], result->p_treatment_less_than_5_to, config->number_of_locations());
  read_p_treatment(node["p_treatment_for_more_than_5_by_location_to"], result->p_treatment_more_than_5_to, config->number_of_locations());
