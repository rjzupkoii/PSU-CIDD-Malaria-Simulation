...
NovelDrugSwitchingStrategy::NovelDrugSwitchingStrategy(): IStrategy("NovelDrugSwitchingStrategy", NovelDrugSwitching) { }
...
void NovelDrugSwitchingStrategy::update_end_of_time_step() {
  //check and switch
  if (therapy_list[1]->id() != switch_to && Model::DATA_COLLECTOR->current_tf_by_therapy()[therapy_list[0]->id()] >= tf_threshold) {
    ...
    LOG(INFO) << date::year_month_day{Model::SCHEDULER->calendar_date} << ": Switch to novel drug with id " << switch_to;
