...
void ConsoleReporter::after_run() {
  ...
  for (int t_id = 0; t_id < Model::CONFIG->therapy_db().size(); t_id++) {
    ...
    std::cout << "Number of patients (with non-resistant parasite) treated with therapy " << t_id << " (% success) = "
  ...
  std::cout << "EAMU count only clinical caused parasite: " << Model::DATA_COLLECTOR->AMU_for_clinical_caused_parasite()
            << std::endl;
